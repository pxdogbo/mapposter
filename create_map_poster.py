...
def generate_output_filename(city, theme_name, output_format, subdir=None):
    """
    Generate unique output filename with city, theme, and datetime.
    If subdir is given, output is placed in POSTERS_DIR/subdir (e.g. posters/duotone).
    """
    out_dir = os.path.join(POSTERS_DIR, subdir) if subdir else POSTERS_DIR
...
def add_border_to_image(filepath: str, hex_color: str, border_px: int = 80) -> None:
    """
    Add a border around the image using the given hex color (e.g. theme text color).
    Overwrites the file. Requires PIL; no-op if PIL not available.
    """
    if not HAS_PIL:
        return
...
def _draw_city_name_letter_spaced(
    fig, ax, y_axes, city_str, fontproperties, color, spacing_px, zorder=11
):
    """
    Draw city name character by character with spacing_px pixels between letters (centered).
    """
    try:
    ...
    axes_width_px = fig_w_inch * fig.dpi * ax.get_position().width
    pixel_to_axes = 1.0 / axes_width_px
    gap_axes = spacing_px * pixel_to_axes

    # Measure each character and compute total width so we can center
    char_widths_axes = []
    for c in city_str:
        t = ax.text(0, 0, c, fontproperties=fontproperties, transform=ax.transAxes)
...
def create_poster(
    ...
    if parks is not None and not parks.empty:
        ...
        if not parks_polys.empty:
            ...
            parks_polys.plot(
                ax=ax,
                facecolor=THEME.get("parks_fill", THEME["bg"]),
                edgecolor="none",
                zorder=0.8,
                alpha=0.6,
            )
    ...
    # Format city name based on script type and letter_spacing
    # letter_spacing: "normal" = font kerning; int/float = pixels between letters (e.g. 40); string = char between letters; None = double space
    if is_latin_script(display_city):
        ...
        if use_pixel_spacing:
            ...
            spaced_city = letter_spacing.join(list(city_display_str))
        else:
            spaced_city = "  ".join(list(city_display_str))
    else:
        city_display_str = display_city
        spaced_city = display_city
        use_pixel_spacing = False
    ...
    # --- BOTTOM TEXT --- (4 rows; vertical spacing proportional to height so full poster matches live preview)
    dpi = 300
...
Examples:
  ...
  python create_map_poster.py --list-themes
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Save under posters/<subdir>/ (e.g. duotone, monotone)",
    )
    parser.add_argument(
        "--no-bordered",
        action="store_true",
        dest="no_bordered",
        help="Only generate the main poster; skip the bordered version (default: generate both)",
    )
    parser.add_argument(
        "--replicate",
        "-r",
        action="store_true",
        help="Post-process poster through Replicate AI model (e.g. dream.district). Requires PNG output.",
    )
    parser.add_argument(
        "--replicate-model",
        type=str,
        default="pxdogbo/dream.district:snedjsxd2xrmr0cwb3hag5kkxw",
        help="Replicate model ID with optional version (default: pxdogbo/dream.district:snedjsxd2xrmr0cwb3hag5kkxw)",
    )
    parser.add_argument(
        "--replicate-prompt",
        type=str,
        default=None,
        help="Prompt for Replicate model (default: minimalist map poster description)",
    )
    parser.add_argument(
        "--replicate-schema",
        action="store_true",
        help="Print Replicate model input schema and exit (to check param names)",
    )
    ...
    try:
        ...
        for theme_name in themes_to_generate:
            ...
            output_file = generate_output_filename(
                args.city, theme_name, args.format, subdir=args.subdir
            )
            create_poster(
                args.city,
                args.country,
            ...
            # Default: also create a bordered version (PNG only)
            if not args.no_bordered and args.format == "png" and HAS_PIL:
                stem = str(Path(output_file).stem)
                parent = Path(output_file).parent
                bordered_path = parent / f"{stem}_bordered.png"
                shutil.copy2(output_file, bordered_path)
                add_border_to_image(str(bordered_path), THEME.get("text", "#FFFFFF"))
                print(f"  ✓ Bordered version saved: {bordered_path.name}")
