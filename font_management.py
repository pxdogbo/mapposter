...
TELEGRAF_DIR = Path(FONTS_DIR) / "Telegraf"


def load_telegraf() -> Optional[dict]:
    """
    Load Telegraf font from local fonts/Telegraf/ directory.
    Expects Telegraf-Regular.otf and Telegraf-UltraBold.otf (or .ttf).
    Returns dict with 'light', 'regular', 'bold' keys, or None if not found.
    """
    if not TELEGRAF_DIR.exists():
        return None
    regular = None
    bold = None
    for f in TELEGRAF_DIR.iterdir():
        if f.suffix.lower() not in (".otf", ".ttf"):
            continue
        name = f.stem.lower()
        if "regular" in name:
            regular = str(f)
        if "ultrabold" in name or ("bold" in name and "ultrabold" not in name):
            bold = str(f)
    if not regular:
        for f in TELEGRAF_DIR.iterdir():
...
def load_fonts(font_family: Optional[str] = None) -> Optional[dict]:
    ...
    """
    # If custom font family specified, try local Telegraf first then Google Fonts
    if font_family and font_family.lower() != "roboto":
        if font_family.lower() == "telegraf":
            fonts = load_telegraf()
            if fonts:
                print("✓ Telegraf font loaded from fonts/Telegraf/")
                return fonts
        else:
